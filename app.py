...
# Some of the code is taken from OAuth course lectures for sign in using Google
# or Facebook with slight modifications.
...
# Generates a random alpha-numeric string which is kept in login_session as state
# and passed with call from client to server to prevent CSRF
def generateRandom():
...
# Based on email provided, it queries the database for the user having same email.
...
# variable called STATE which has the randomly generated value. render_template
# function replaces STATE variable in login.html file, whereever it is referenced
# like this {{STATE}}
...
# This method implements OAuth2 using Google Sign in. It also adds the new user
# to database. This code is taken from
...
# from templates folder and replaces STATE, USERNAME, PICTURE according to context
# variables being set in render_template method arguments
@app.route('/')
@app.route('/index')
def index():
    if 'username' not in login_session:
        ...
        if request.args.get('state') != login_session['state']:
            ...
            response.headers['Content-Type'] = 'application/json'
            return response
        return render_template('index.html', STATE=login_session['state'], USERNAME=login_session['username'], PICTURE=login_session['picture'])
...
# <int:category_id> means that this method expects the URL to have a category id
# Queries the database,
#     1 - for retreiving the items added by user only,
#     2 - for retreiving the items added by other users
# Returns the response as JSON object with two properties ie. userItems, itemsByOtherUsers
...
def getAllItemsByCategoryId(category_id):
    if 'username' not in login_session:
        ...
        userItems = session.query(Item).filter(and_(Item.category_id==category.id, Item.user_id == login_session['user_id']))
        itemsByOtherUsers = session.query(Item).filter(and_(Item.category_id==category.id, Item.user_id != login_session['user_id']))
        return jsonify(userItems=[i.serialize for i in userItems], itemsByOtherUsers=[j.serialize for j in itemsByOtherUsers])
...
def getItemByCategoryIdAndItemId(category_id, item_id):
    if 'username' not in login_session:
        ...
        if request.args.get('state') != login_session['state']:
            ...
            return response
        category = session.query(Category).filter_by(id=category_id).one()
        items = session.query(Item).filter(and_(Item.category_id==category.id, Item.id==item_id, Item.user_id == login_session['user_id']))
...
# Adds a new item against the category id specified in <int:category_id>
# Loads data from POST request formdata
    ...
    if 'username' not in login_session:
        ...
        if request.method == 'POST':
            try:
                ...
                # extension, then appends a date-time string with the file name
                # to keep it uniques and prevent overwriting of any other user's
                # file with same name
                ...
                item = Item(name=name, description=description,
                            price=price, image_src=image_src, category=category, user_id=login_session['user_id'])
                session.add(item)
...
# Handles editing of an existing item which has valid category id and item id and 
# user had added the item previously.
@app.route('/edititem/category/<int:category_id>/item/<int:item_id>', methods=['GET','POST'])
        ...
        if request.method == 'POST':
            try:
                ...
                item = session.query(Item).filter(
                    and_(Item.category_id==category_id, Item.id==item_id, Item.user_id == login_session['user_id'])).one()
                if item:
                    if image_src:
                        item.name = item_name
                        ...
                        item.description=item_description
                        item.user_id = login_session['user_id']
                        item.price=item_price  # Do not change the image unless
                                                 # user provides a new image
                    session.add(item)
                    session.commit()
                    return jsonify(updated=True,msg='Record has been updated')
                else:
                    return jsonify(updated=False,msg='No such item exist')
            except Exception,e:
                print str(e)
                return jsonify(updated=False, msg='An error occured while trying to update this item.')
        else:
            return jsonify(updated=False,msg='An error occured while trying to update this item.')
    ...
    if 'username' not in login_session:
        ...
        if request.args.get('state') != login_session['state']:
            ...
            try:
                ...
                print str(e)
                return jsonify(deleted=False,msg='An error occured while trying to delete this item.')
